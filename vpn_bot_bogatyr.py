...
PRICES = {
    '1-month': {'price': 150, 'title': '1 месяц', 'description': 'Bogatyr VPN на 1 месяц'},
    '2-months': {'price': 250, 'title': '2 месяца', 'description': 'Bogatyr VPN на 2 месяца'},
    '4-months': {'price': 400, 'title': '4 месяца', 'description': 'Bogatyr VPN на 4 месяца'},
    '6-months': {'price': 500, 'title': '6 месяцев', 'description': 'Bogatyr VPN на 6 месяцев'},
}
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if data.get("used_traffic", 0) == 0:
                bot.send_message(
                    user_id, 
                    "👋 Вижу, вы до сих пор не начали использовать VPN. У вас возникли какие-то проблемы? Напишите в нашу поддержку: @suppbogatyr",
                    parse_mode="HTML"
                )
    except Exception as e:
...
def give_vpn_access(user_id: int, days: int, reason: str):
    ...
    try:
        vless_link = VLESS_TEMPLATE.format(
            uuid=uuid,
            label=username.split('_')[0] 
        )
    except Exception as e:
        bot.send_message(
            user_id,
            "❌ Ошибка при выдаче ключа. Напишите в поддержку: @suppbogatyr",
        )
...
"""
    ...
    btn_support = types.InlineKeyboardButton(
        text="💬 Поддержка",
        url="https://t.me/suppbogatyr"
    )

    btn_guide = types.InlineKeyboardButton(
        text="📱 Инструкция по подключению",
        url="https://telegra.ph/ZAMENIT-NA-GAJD-BOGATYR" # <-- ТУТ НУЖНО ЗАМЕНИТЬ НА НОВЫЙ ГАЙД BOGATYR VPN
    )
...
def start(message):
    ...
    welcome_text = """
«Здравствуй, добрый молодец (аль девица красная)!

Встала над Русью сеть невидимая, оковы заморские да преграды цифровые. Но не горюй! Bogatyr VPN — твой верный щит и меч в мире интернета. Прорубим окно в любые земли, да скроем след твой от глаз недобрых!

💨 Скорость былинная — быстрее ветра в чистом поле!
👾 Крепость неприступная — никакая стража доступ не закроет!
🗓 Три дня пробы даром — испытай силушку нашу бесплатно!

👫 Позови дружину свою! За каждого ратника, что по совету твоему придет, получишь 60% золотом от его вклада!

📌 Указ обязательный:
Чтобы сила работала, <a href='https://t.me/bogatyrvpn'>вступи в наш Telegram-канал</a>.

⚡️ Ковка ключа мгновенная — сразу после оплаты!
Гонец для связи (поддержка): <a href='https://t.me/suppbogatyr'>Bogatyr Support</a>»
    """
...
def legal_info(call):
    ...
    btn_policy = types.InlineKeyboardButton(
        ...
        url="https://telegra.ph/Politika-konfidencialnosti-02-20-26"  
    )
    btn_offer = types.InlineKeyboardButton(
        text="📃 Договор оферты",
        url="https://telegra.ph/YUridicheskaya-informaciya-02-20"
    )
...
def handle_free_trial(call):
    ...
    if not is_subscribed(user.id):
        keyboard = types.InlineKeyboardMarkup()
        btn_channel = types.InlineKeyboardButton("📢 Подписаться на канал", url="https://t.me/bogatyrvpn")
...
def process_payment(call):
    ...
    try:
        ...
        pass
    print(YOOKASSA_TOKEN)
    bot.send_invoice(
        chat_id=call.message.chat.id,
        title=f"Bogatyr VPN - {price_info['title']}",
...
def process_successful_payment(message):
    ...
    if days:
        ...
        if referrer_id and not users[uid].get("ref_bonus_paid", False):
            ...
            if ref_uid in users:
                reward = round(total_amount * 0.6, 2)  
